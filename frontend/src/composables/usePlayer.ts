...
//   useSettings.ts — 歌词/UI/桌面歌词三套设置 + 主题应用
//   useEq.ts       — 均衡器对外 API（预设/滑杆/校验）
//   useLyric.ts    — 歌词加载/手动歌词/行定位/跟唱锚点
//   useAbLoop.ts   — AB 区间循环 + 单句循环 + 跟唱句末处理
//   useLibrary.ts  — 收藏/歌单
// 保留本文件作为聚合出口：现有组件与测试的 import 路径不变；
// 后续新任务直接 import 具体模块（如 AB 循环改 useAbLoop.ts）。
...
export * from "./useEq.ts";
export * from "./useLyric.ts";
export * from "./useAbLoop.ts";
export * from "./useLibrary.ts";
