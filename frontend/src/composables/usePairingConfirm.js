...
const current = ref(null); // 当前待确认请求 {request_id, device_name, device_type, created_at}
const busy = ref(false); // approve/reject 请求中（防重复提交）

// 非响应式内部态
const queue = []; // 待确认队列（逐个弹）
const seenIds = new Set(); // 已见过（含已处理）的 request_id，防止重复弹
 ...
 */
export function isPairingEnabled() {
  try {
...
export async function _pollOnce() {
  const r = await apiGet("/api/pairing/pending");
...
async function handleAction(approve) {
  if (!current.value || busy.value) return false;
  const req = current.value;
  busy.value = true;
  const action = approve ? "approve" : "reject";
  const r = await apiPost(`/api/pairing/request/${encodeURIComponent(req.request_id)}/${action}`);
  busy.value = false;
  if (r.ok) {
    closeCurrent();
 ...
 */
export function usePairingConfirm() {
  startPolling();
...
export function _resetPairingConfirm() {
  stopPolling();
  visible.value = false;
  current.value = null;
  busy.value = false;
