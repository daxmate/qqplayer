...
export function readBarData(a: AnalyserNode, bars: number): number[] {
...
export function readWaveData(a: AnalyserNode, count: number): number[] {
...
function ensurePeaks(n: number): void {
  if (peaksCount !== n || peaks.length !== n) {
    peaksCount = n;
    peaks = new Array<number>(n).fill(0);
  }
}
export function _resetPeaks(): void {
  peaks = [];
  peaksCount = 0;
}
...
/** 视觉化绘制选项（颜色由组件读取 CSS 变量传入；small/dpr 供粒子性能降级） */
interface VisualizerOpts {
  accent?: string;
  accent2?: string;
  small?: boolean;
  dpr?: number;
...
export function drawWave(
  ...
  const trace = (close: boolean): void => {
...
function idleWave(
  ...
  const sample = (fn: CanvasPath): void => {
    for (let i = 0; i < pts; i++) {
      const x = (i / (pts - 1)) * w;
...
function ensureParticles(count: number, w: number, h: number): void {
...
export function _resetParticles(): void {
  particles = [];
  particlesKey = "";
  pTime = 0;
}
...
export function _resetVisualizer(): void {
  analyser = null;
  attached = false;
  failed = false;
}
...
const colorCache = new Map<string, Promise<RGB | null>>();
...
export function extractCoverColor(src: string): Promise<RGB | null> {
