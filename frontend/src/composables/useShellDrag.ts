...
/** setupShellRowDrag 参数（Playlist.vue / SmartViewPanel.vue 在壳内时调用） */
interface ShellRowDragOptions {
  /** 滚动容器（pointerdown 委托挂在这里） */
  listEl: HTMLElement;
  /** 行选择器（rows() 内部用它；默认 .pl-item） */
  rowSelector?: string;
  /** 拖拽手柄选择器（pointerdown 命中手柄才进入拖拽；默认 .pl-drag） */
  handleSelector?: string;
  /** 拖拽启用条件（false = 手柄按下不进入拖拽态） */
  getCanDrag: () => boolean;
  /** 列表内排序允许条件（false = 拖到侧栏歌单仍生效，列表内不显示插入线、不排序；默认 true） */
  getCanReorder?: () => boolean;
  /** 歌单视图（true = setPlaylistOrder，false = reorderQueue） */
  isPlaylistView: () => boolean;
  /** 全部歌曲视图重排（reorderQueue + persistQueueOrder） */
  onQueueReorder: (from: number, to: number) => void;
  /** 歌单视图重排（setPlaylistOrder） */
  onPlaylistReorder: (paths: string[]) => void;
}
...
export function setupShellRowDrag({
  ...
  onQueueReorder,
  onPlaylistReorder,
}: ShellRowDragOptions): (() => void) | null {
  if (!inNativeShell() || !listEl) return null;

  let dragging = false; // 是否超过阈值进入拖拽态
  let pointerId: number | null = null; // 拖拽指针 id（多点触控只跟踪第一个）
  let startX = 0;
  let startY = 0;
  let startScrollTop = 0;
  let sourceEl: HTMLElement | null = null; // 被拖行
  let sourceIndex = -1; // 被拖行在 DOM 中的索引（拖拽期间 DOM 不重排，稳定）
  let sourcePath: string | null = null; // 被拖歌曲路径（网络歌 null → 不能加歌单，可排序）
  let sourceBaseCenter = 0; // 被拖行起始中心 Y（行跟随指针 = base + clientY 位移）
  let targetIndex = -1; // 当前插入索引（-1 = 无目标）
  let hoverPlaylistEl: HTMLElement | null = null; // 当前悬停的歌单项（sb-drop 高亮）

  const rows = (): HTMLElement[] => [...listEl.querySelectorAll<HTMLElement>(rowSelector)];

  // 行中心交叉模型算插入索引：被拖行中心（跟随指针）越过某行中心 → 插到该行之后。
  // 语义与 SortableJS onEnd 的 newIndex 一致（移除源行后的位置）；其他行 rect 每次重取，
  // 列表滚动时中心随内容移动，天然正确。
  function computeTarget(clientY: number): number {
  ...
  function hitSidebarItem(x: number, y: number): HTMLElement | null {
    for (const el of document.querySelectorAll<HTMLElement>(".sb-item[data-playlist-id]")) {
  ...
  function applyIndicator(t: number) {
    clearIndicator();
    if (t < 0) return;
    const all = rows();
    const n = all.length;
  ...
  function onPointerDown(e: PointerEvent) {
    ...
    if (!handle) return;
    const el = handle.closest(rowSelector) as HTMLElement | null; // 被拖行是 .pl-item 行（HTMLElement）
    if (!el) return;
  ...
  function onPointerUp(e: PointerEvent) {
    ...
    if (isPlaylistView()) {
      ...
      const idx = sourcePath != null ? paths.indexOf(sourcePath) : -1;
  ...
  let clickCapture: ((ev: Event) => void) | null = null;
