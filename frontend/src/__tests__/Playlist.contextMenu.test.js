...
function mountSongs(songs = SONG) {
...
const menuEl = () => document.body.querySelector(".ctx-menu");
const menuText = () => menuEl()?.textContent || "";

function menuItem(text) {
  const btns = [...document.body.querySelectorAll(".ctx-item")];
  return (
...
describe("Playlist 移到废纸篓", () => {
  it("批量删除：确认弹窗 → DELETE → toast 汇总 → 队列清理 + 当前歌切下一首 → loadSongs 刷新", async () => {
    ...
    // 取消 → 不调 DELETE
    await modal.querySelector(".dt-btn").click();
    await nextTick();
    ...
    await wrapper.findAll(".pl-multi-btn")[2].trigger("click");
    await nextTick();
    await document.body.querySelector(".dt-modal .dt-btn.danger").click();
    await flushPromises();
    const delCall = fetchMock.mock.calls.find(([u]) => String(u).includes("/api/library/songs"));
    expect(delCall).toBeTruthy();
    expect(delCall[1].method).toBe("DELETE");
    expect(JSON.parse(delCall[1].body).paths).toEqual(["/a.mp3", "/c.mp3"]);
    // toast 汇总：已删除 + 不在曲库
