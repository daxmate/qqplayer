...
const memCache = new Map(); // key → {value, ts, ttl}
let memOpsSeq = 1;
const memOps = []; // [{id, op, payload, ts}]
 ...
 * @returns {*} 命中返回缓存 value，未命中返回 null
 */
export function getCache(key, { maxAge } = {}) {
  ...
  return tx(CACHE_STORE, "readonly", (store) => store.get(key)).then((row) => {
    if (!row) return null;
...
export function setCache(key, value, ttl = 0) {
  const row = { key, value, ts: Date.now(), ttl };
  if (!canUseIndexedDB()) {
    memCache.set(key, row);
    return Promise.resolve();
  }
  return tx(CACHE_STORE, "readwrite", (store) => store.put(row));
}
...
export function delCache(key) {
  ...
  return tx(CACHE_STORE, "readwrite", (store) => store.delete(key));
}
...
export function clearCache() {
  ...
  return tx(CACHE_STORE, "readwrite", (store) => store.clear());
}

// ============ pendingOps 表：写路径 dirty 队列 ============

/** 入队写操作 → 返回队列 id。op 为 {url, method}，payload 为请求体（JSON 对象） */
export function enqueuePendingOp(op, payload) {
  const row = { op, payload, ts: Date.now() };
  if (!canUseIndexedDB()) {
    row.id = memOpsSeq++;
    memOps.push(row);
  ...
  return tx(OPS_STORE, "readwrite", (store) => store.add(row)).then((id) => id ?? undefined);
}

/** 全部待同步操作（按入队顺序） */
export function getPendingOps() {
  if (!canUseIndexedDB()) return Promise.resolve([...memOps]);
  return tx(OPS_STORE, "readonly", (store) => store.getAll()).then((rows) => rows || []);
}

/** 按 id 批量移除（成功同步后清队） */
export function removePendingOps(ids) {
  const list = Array.isArray(ids) ? ids : [ids];
  if (!canUseIndexedDB()) {
    for (let i = memOps.length - 1; i >= 0; i--) {
      if (list.includes(memOps[i].id)) memOps.splice(i, 1);
    }
    return Promise.resolve();
  }
  return tx(OPS_STORE, "readwrite", (store) => {
...
export function clearPendingOps() {
  ...
  return tx(OPS_STORE, "readwrite", (store) => store.clear());
}
