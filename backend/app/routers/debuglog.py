...
- POST /api/debuglog {line}  追加一行（客户端上报：锁屏/线控命令到达时间线）
- GET  /api/debuglog         返回全部缓冲行（排查时直接 curl 读取）

用途：锁屏/后台场景下 WebView 里的 JS 可能被系统挂起，前端日志不可达；
原生侧不依赖 WebView，直接把关键决策 POST 到这里，排故时从主机端一眼看到
「命令是否到达原生、何时到达」。（原为 iOS 壳诊断端点，壳 2026-09-13 退役后保留。）
